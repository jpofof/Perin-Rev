...
**Data:** 20-21/07/2026
**Contexto:** apesar de todas as correções anteriores deste relatório terem melhorado métricas medidas via Lighthouse/WebPageTest/Puppeteer (todos baseados em Chrome/Blink, mesmo em "modo mobile"), o usuário e os pais dele continuaram sentindo travamento real ao usar o site no Safari do iPhone — um sintoma que nunca apareceu em nenhuma ferramenta de medição usada até então. Esta seção resume a investigação completa, feita sem acesso a profiling real do Safari (sem Mac, `ios-webkit-debug-proxy` incompatível com iOS moderno), até a causa raiz confirmada e a correção aplicada.

### Passo 1 — Instrumentação de performance real no dispositivo (`?debug=perf`)

Como nenhuma ferramenta de medição baseada em Chrome reproduzia o problema, a estratégia foi instrumentar a própria página (reaproveitando o padrão já validado no bug de salto de scroll da Fase 1: ativação por query param, zero custo em produção, exportação via botão de clipboard) para capturar dados reais direto no iPhone do usuário.

Capturado: frame timing via `requestAnimationFrame` (deltas > 50ms), long tasks via `PerformanceObserver` (com fallback explícito quando indisponível no Safari), heartbeat de `setInterval` de 200ms (gap > 500ms = main thread bloqueado tempo suficiente para o próprio timer atrasar), e marcos de ciclo de vida (hero entrance, Grupo A, primeira imagem do carrossel de clientes).

**Resultado real capturado no iPhone:** bloqueio de main thread de **~3785ms sem nenhum frame**, heartbeat atrasado em **~3843ms**, ocorrendo entre `DOMContentLoaded` (t=44ms) e t≈3839ms — antes do Grupo A rodar, antes do `load`, ainda durante a timeline de entrada do hero (que levou mais de 5s no total, quando o esperado era ~1,5-2s). Confirmado pelo usuário como determinístico (reprodutível em toda carga, não é aquecimento/JIT).

### Passo 2 — Checkpoints granulares no bloco síncrono de `initPage()`

Adicionados checkpoints `start`/`end` em volta de cada uma das 5 chamadas síncronas reais de `initPage()` (`initHeroVideoBackground`, `initHeroParallax`, `initHeroAnimations`, `initNavigation`, `initButtonRipple` — lista confirmada por inspeção de código, corrigindo a hipótese inicial que incluía `createParticles`, que na verdade roda depois, na fila idle). Objetivo: achar qual função (ou soma delas) consumia os ~3,8s.

### Passo 3 — Investigação e descarte do padrão periódico de ~3s

O usuário reportou que o travamento se repetia em intervalos de ~3s durante **toda a sessão**, não só no carregamento — coincidindo suspeitosamente com `animation: particleFloat 3s infinite` das 50 partículas do hero. Investigação por inspeção de código:

- **Descartado:** nenhum listener `animationiteration`/`animationend` anexado às partículas — a animação é 100% CSS, sem callback JS, e a duração real por partícula é aleatória (3-7s), não fixa em 3s.
- **Descartado:** nenhum `setInterval` de produção no código (só os 2 da própria instrumentação de debug).
- **Descartado como incompatível:** o loop forward/reverse do vídeo do hero é periódico, mas com ciclo de ~16,2s (confirmado via `ffprobe`), não ~3s.
- Adicionada interceptação ampla de `window.setTimeout`/`window.setInterval` (log de criação/disparo para delays entre 2500-3500ms) para cobrir qualquer timer não previsto, incluindo de bibliotecas de terceiro (GSAP). Validação local não achou nenhum timer periódico nessa janela além do fallback conhecido (one-shot, 3,5s).

### Passo 4 — Isolamento por eliminação via `?isolate=...`

Sem acesso a Safari Web Inspector remoto, a estratégia seguinte foi desligar componentes suspeitos por completo (não só pausar), um de cada vez, via query param, e pedir para o usuário testar cada versão no iPhone real:

| Isolamento | O que desligava | Resultado no iPhone |
|---|---|---|
| `no-video` | Vídeo timelapse do hero | Travamento idêntico — **não ajuda** |
| `no-particles` | 50 partículas CSS do hero | Travamento idêntico — **não ajuda** |
| `no-scrolltrigger-only` | Só o ScrollTrigger (reveals ao rolar), timeline do hero intacta | Melhora **bem pouco** |
| `no-gsap` | Toda a timeline de entrada GSAP + ScrollTrigger + carrossel cascata do portfólio | **Quase elimina** o travamento |
| `no-geometries` | Grade de fundo + SVGs geométricos + luzes com blur do hero (`hero-grid`/`hero-geometries`/`hero-lighting`), mantendo a timeline GSAP do texto intacta | **Elimina o travamento por completo** |

A diferença entre `no-gsap` e `no-scrolltrigger-only` apontou a própria execução da timeline de entrada como fator determinante — mas `no-geometries` isolou a causa de forma definitiva, revelando que as camadas de geometria/grid/luz (que, por inspeção de código, **não são animadas pela timeline GSAP** — rodam via `@keyframes floatShape` em CSS puro e via parallax do mouse em JS vanilla, ambos independentes do GSAP) eram o verdadeiro fator, não a timeline em si.

### Passo 5 — Separação entre blur estático e animação (`no-blur-only` / `no-animation-only`)

Com a causa confinada às 3 camadas de `no-geometries`, dois isolamentos adicionais separaram duas características possíveis: os 3 `.light-spot` usam `filter: blur(100px)` (conhecido por ser caro no compositor do WebKit/Safari mesmo parado) e as formas usam animação contínua (CSS `floatShape` + parallax do mouse).

- `no-blur-only` — formas nítidas (sem blur), **animando normalmente**.
- `no-animation-only` — formas com blur original de 100px, **completamente paradas**.

**Resultado confirmado no iPhone: `no-blur-only`, sozinho, já resolve o travamento por completo, com a animação mantida normal.** Causa raiz isolada: `filter: blur(100px)` nos 3 `.light-spot`.

### Causa raiz confirmada

**`filter: blur(100px)`** aplicado aos 3 elementos `.light-spot` do hero (`styles.css`) — um blur pesado (raio de 100px) sobre elementos de até 600×600px, com custo de composição no motor WebKit/Safari desproporcionalmente maior do que no Chrome/Blink (por isso nunca apareceu em nenhuma medição baseada em Chrome ao longo de todo este relatório). A animação CSS das formas, o parallax do mouse, o vídeo, as partículas e o ScrollTrigger foram todos testados e descartados individualmente.

### Correção aplicada

`styles.css`, regra `.light-spot`: `filter: blur(100px)` → **`filter: blur(24px)`**, permanente (sem condicional `?isolate=`), válido para mobile e desktop — o teste de isolamento não indicou necessidade de manter o blur pesado em nenhum dos dois. O valor de 24px foi escolhido dentro da faixa sugerida (20-30px) por preservar o efeito de "glow" suave pretendido no design, reduzindo drasticamente o custo de composição do valor original. Nenhuma outra parte do hero (timeline GSAP, animação `floatShape`, parallax do mouse) foi alterada — todas já confirmadas como não sendo a causa.

Toda a infraestrutura de diagnóstico `?isolate=...` (`no-video`, `no-gsap`, `no-particles`, `no-carousel-clients`, `no-scrolltrigger-only`, `no-geometries`, `no-blur-only`, `no-animation-only`, `minimal`) foi removida do código, já tendo cumprido seu propósito. A instrumentação `?debug=scroll`/`?debug=perf` da Fase 1/2 foi mantida (útil para validações futuras).

### Validação

- `npm test`: 4 suítes, **112 testes passando** (unitários + 65 de regressão do carrossel) — comportamento normal do site inalterado.
- Comparação visual (Puppeteer, antes/depois): hero visualmente idêntico — o glow dos `light-spot` tem opacidade baixa (0,1) e é majoritariamente coberto pelo vídeo do hero, então a redução de blur não introduz nenhuma diferença perceptível de layout ou conteúdo.
- `script.min.js`/`styles.min.css` regenerados e confirmados (via inspeção do arquivo publicável) contendo `filter:blur(24px)` — o que será servido em produção.
