# Perin Construções — Site Institucional Premium

Site institucional de alto padrão para a **Perin Construções**, empresa de engenharia e arquitetura especializada em construção residencial, comercial, industrial, reformas, ampliações, gerenciamento de obras e consultoria técnica.

O projeto foi desenvolvido para transmitir engenharia, arquitetura, sofisticação, tecnologia, precisão, robustez e alto padrão — uma experiência digital premium que posiciona a Perin Construções como uma empresa capaz de executar grandes projetos.

---

## 🚀 Tecnologias

| Tecnologia | Versão | Finalidade |
|---|---|---|
| **HTML5** | — | Estrutura semântica e acessível (ARIA labels, roles, landmarks) |
| **CSS3** | — | Design System próprio com Design Tokens (`:root`) |
| **JavaScript Vanilla** | ES6+ | Engine de animações e interações (sem frameworks) |
| **GSAP** | 3.12.5 | Animações de alta performance |
| **GSAP ScrollTrigger** | 3.12.5 | Scroll reveals, scrubs, pinning |
| **Bootstrap** | 5.3.3 | Grid utilitário (uso mínimo) |
| ~~SwiperJS~~ | — | Removido do código (CDN e uso); substituído pelo Cascading Slider próprio |

> **Nota:** Lenis (smooth scroll) foi removido por conflitos de scroll. O scroll nativo com GSAP ScrollTrigger provê animações fluidas sem comprometer a usabilidade.

---

## 📁 Estrutura do Projeto

```
/
├── index.html                          # Página principal (9 seções)
├── styles.css / styles.min.css         # Design System completo (fonte / minificado, servido em produção)
├── script.js / script.min.js           # Engine de animações e interações (fonte / minificado)
├── 404.html, politica-de-privacidade.html
├── AGENTS.md                           # Contrato de comportamento do agente de IA (fonte da verdade)
├── CLAUDE.md                           # Overrides específicos do Claude Code (referencia AGENTS.md)
├── README.md                           # Documentação (este arquivo)
├── _headers                            # CSP e demais headers de segurança/cache (Netlify)
├── netlify.toml                        # Configuração de build/publish do Netlify
├── robots.txt / sitemap.xml / llms.txt
├── package.json / jest.config.js       # Dependências e configuração da suíte de testes (Jest)
│
├── assets/
│   └── images/
│       ├── clients/                    # Logos de clientes (elektro, isa-energia, state-grid)
│       ├── brand/                      # Logos/favicons da própria Perin
│       ├── about/, hero-fundo/         # Imagens das seções Sobre e Hero
│       ├── social/                     # Imagem de preview para og:image/twitter:image
│       ├── placeholders/               # placeholder-obra-03.webp — cover em uso no carrossel (script.js)
│       └── portfolio/                  # Fotos reais de obras (parcial, ex.: elektro-02..06.webp) — uso pontual
│
├── fonts/                              # Web fonts self-hosted (.woff2)
├── vendor/                             # GSAP e Lenis self-hosted (evita CDN externo)
│
├── docs/
│   ├── produto/                        # PRD.md, SPECS.md
│   ├── planejamento/                   # PLANO-MELHORIAS.md e notas técnicas
│   ├── relatorios/                     # AUDITORIA.md, RELATORIO-PERFORMANCE.md (histórico de perf) etc.
│   └── ferramentas/                    # Notas sobre graphify, headroom
│
├── audit/                              # Investigações pontuais (Lighthouse, diagnósticos de produção)
├── prototipos/                         # Referências externas testadas isoladamente (ex.: cascading-slider/)
├── _archive/                           # Código/assets legados, fora do build ativo
├── scripts/
│   ├── build-netlify.js                # Monta dist/ com só o necessário p/ produção + cache-busting (?v=hash)
│   ├── check-min-freshness.js          # Verifica se .min.js/.min.css estão atualizados vs. fonte
│   └── install-git-hooks.js
│
├── graphify-out/                       # Grafo de conhecimento do código (gerado, ver .claude/skills/graphify)
│
└── tests/
    ├── unit/                           # slider, clients-carousel, form, init-scheduling
    └── regression/                     # slider, process-diagram (Puppeteer)
```

Build de produção: `npm run build:netlify` gera `dist/` (gitignored) copiando somente os arquivos referenciados acima (não inclui `docs/`, `audit/`, `scripts/`, `tests/`, `prototipos/`, `_archive/`) e aplica cache-busting — anexa `?v=<hash-md5-do-conteúdo>` em `script.min.js`/`styles.min.css` no `index.html` gerado, evitando que o `Cache-Control: max-age=2592000` do `_headers` sirva JS/CSS desatualizado por até 30 dias após um deploy.

---

## 🎨 Design System

### Paleta de Cores (Design Tokens)

Todas as cores no arquivo `styles.css`:

```css
:root {
    /* Tons Claros / Fundos */
    --cor-branco-gelo: #F5F5F5;           /* Texto principal */
    --cor-creme: #F7F7DF;                 /* Reserva */

    /* Tons de Verde (Destaques e Elementos) */
    --cor-verde-neon: #0CEB82;            /* Glow / gradientes */
    --cor-verde-brilhante: #3FCC5B;       /* Hover */
    --cor-verde-floresta: #2A873E;        /* Cor primária */

    /* Tons Neutros / Base e Textos */
    --cor-bege-escuro: #BDB49F;           /* Texto secundário */
    --cor-cinza-acastanhado: #857F76;     /* Texto muted */
    --cor-preto-puro: #0A0B0B;            /* Fundo principal */
}
```

### Tipografia

- **Headings:** Manrope (800, 700, 600) — pesos bold para títulos
- **Body:** Inter (400, 500) — legibilidade premium
- **Proibido:** Poppins, Roboto, Open Sans, Montserrat como principal

### Sistema Tipográfico

| Elemento | Tamanho | Peso |
|---|---|---|
| Hero | 72px (clamp) | 800 |
| Título de Seção | 56px (clamp) | 700 |
| Subtítulo | 24px | 500 |
| Texto | 18px | 400 |

---

## 📄 Seções do Site

O `index.html` contém **9 seções** estruturadas:

| # | Seção | Classe CSS | ID |
|---|---|---|---|
| 1 | **Preloader** | `.premium-preloader` | `#preloader` |
| 2 | **Navegação** | `.premium-navigation` | `#mainNav` |
| 3 | **Hero** | `.hero-architectural-scene` | `#hero` |
| 4 | **Sobre** | `.about-narrative-section` | `#about` |
| 5 | **Serviços** | `.services-interactive-module` | `#services` |
| 6 | **Processo** | `.process-journey-section` | `#process` |
| 7 | **Portfólio** | `.portfolio-gallery-section` | `#portfolio` |
| 8 | **Diferenciais** | `.differentials-showcase-section` | `#differentials` |
| 9 | **Depoimentos** | `.testimonials-section` | `#testimonials` |
| 10 | **Contato** | `.premium-contact-experience` | `#contact` |
| 11 | **Footer** | `.premium-footer` | — |

---

## 🎬 Animações e Interações

### Engine (`script.js`)

- **Preloader:** Barra de progresso animada com `setInterval`
- **Hero Particles:** 50 partículas CSS animadas com `@keyframes`
- **Hero Parallax:** Mouse tracking nas geometrias, grid e lighting (3 camadas)
- **Hero Entrance:** Timeline GSAP com rotateX 3D, y translation, opacity
- **Hero Scroll:** Scrub com scale + opacity no canvas + overlay gradient
- **Scroll Reveals:** Section titles, text, process steps (com stagger)
- **Counters:** Animações GSAP com snap para estatísticas
- **Service Interactions:** Hover com scale + rotate nos SVGs
- **Button Ripple:** Efeito ripple nos botões primários
- **Contact Form:** Floating labels com foco, submit feedback animado

### Carrossel de Portfólio (Cascading Slider)

Implementação própria com GSAP. Efeito "janela" — as imagens possuem escala fixa e os slides revelam mais ou menos área da fotografia conforme mudam de tamanho.

#### Estrutura Desktop

```
[6,5%] [13,5%] [60%] [13,5%] [6,5%]
```

5 slots com gap de 4px. Slide central ocupa 60% da largura útil.

#### Estrutura Mobile (<750px)

```
[10%] [80%] [10%]
```

3 slots visíveis. Slides extremos ficam off-screen e entram na sequência durante a navegação.

#### Comportamento

| Característica | Especificação |
|---|---|
| **Animação** | GSAP `to()` com `duration: 0.70s` |
| **Curva** | `cubic-bezier(0.40, 0.00, 0.30, 1.00)` — início ~40% mais suave que linear |
| **Navegação** | Clique nos slides, botões ← →, teclado (ArrowLeft/ArrowRight) |
| **Loop** | Circular infinito — slides off-screen participam da sequência |
| **Imagens** | Escala fixa (`width: auto; height: 100%`), centralizadas com `translate(-50%, -50%)` |
| **Overlay** | Transparente — sem escurecimento dos slides laterais |
| **Filtros** | Nenhum — sem `brightness()` ou `blur()` nos slides |
| **Texto** | Título + categoria visíveis apenas no slide ativo, com fade-in/out |
| **Touch** | Feedback visual (`touch-active`) durante a transição, removido automaticamente ao final |
| **Resize** | Handler debounced (100ms) recalcula posições sem recarregar a página |

#### Especificações Técnicas

```javascript
// Proporções (PCT)
const PCT_DESKTOP = [0.065, 0.135, 0.60, 0.135, 0.065];  // 5 slots
const PCT_MOBILE  = [0.10, 0.80, 0.10];                     // 3 slots (≤750px)

// Animação
const DURATION = 0.70;  // segundos
const CURVE = 'cubic-bezier(0.40, 0.00, 0.30, 1.00)';

// Altura
ch = isMobile() ? Math.min(window.innerHeight * 0.45, 340) : 420;  // px
```

#### Regras de Imagem (CSS)

```css
.cascading-slide-image img {
    width: auto;           /* escala fixa — NUNCA 100% */
    height: 100%;          /* mesma altura em todos os slides */
    max-width: none;
    object-fit: cover;
    position: absolute;
    left: 50%; top: 50%;
    transform: translate(-50%, -50%);  /* centralizado */
}
```

A largura em pixels é definida via JS (`fixedImgWidth`) baseada no slide central (60% desktop / 80% mobile), garantindo que a imagem nunca mude de escala durante a navegação — o slide funciona como uma janela que revela mais ou menos área da fotografia.
</replace_in_file>

---

## 🧩 Nomenclatura

### Convenção de classes (kebab-case)

Todas as classes CSS seguem o padrão **kebab-case** para fácil edição:

```css
.hero-architectural-scene    /* ✅ correto */
.service-mosaic-item         /* ✅ correto */
.form-floating-input         /* ✅ correto */
```

IDs seguem **camelCase** para referência no JavaScript:

```javascript
document.getElementById('cascadingSliderList')  // ✅ correto
```

### Nomes proibidos

`section1`, `card1`, `box1`, `wrapper1`, `container1`, `item1`, `temp`, `test`

---

## 🎯 Personalização

### Cores
Edite o bloco `:root` no início do `styles.css`.

### Conteúdo
Substitua os placeholders `[Inserir ...]` no `index.html` pelos dados reais da empresa:
- Apresentação, missão, visão, valores, diferenciais
- Imagens reais no portfólio e sobre
- Depoimentos reais de clientes
- Email oficial, telefone, endereço
- Estatísticas (projetos realizados, anos de experiência)
- Slogan da empresa

### Carrossel (Portfólio)
No `script.js`, função `initCascadingSlider()`:
- `slideHeight`, `centerWidth`, `sideWidth`, `gap` — dimensões dos slides
- `opacityVal` — opacidade por distância do centro
- `transitionDelay` — delay do texto ao entrar no centro (0.4s)

---

## 🖥️ Responsividade

| Breakpoint | Largura | Ajustes principais |
|---|---|---|
| **Desktop** | >1200px | Layout completo |
| **Tablet** | ≤1024px | Grid 2 colunas em about/valores/diferenciais |
| **Mobile** | ≤768px | Menu hamburger, seções empilhadas, padding reduzido |
| **Small** | ≤480px | Botões full-width, fonte menor no menu mobile |

---

## 📐 Checklist de Qualidade

- [x] HTML semântico com ARIA labels
- [x] Design responsivo (4 breakpoints)
- [x] Animações com GSAP + ScrollTrigger
- [x] Navegação mobile com menu overlay
- [x] Formulário com floating labels e validação visual
- [x] Carrossel de portfólio customizado (cascading slider)
- [x] Placeholders para dados reais (sem invenção)
- [x] Zero dependências de servidor (100% estático)
- [x] Design System com Design Tokens (`:root`)
- [x] Nomenclatura semântica kebab-case
- [x] SEO (meta tags, Open Graph, canonical)
- [x] Performance (lazy-ready, CDNs com versões pinadas)

---

## 🧪 Testes Automatizados

O projeto possui uma suíte de testes unitários e de regressão cobrindo o carrossel de portfólio (cascading slider), vídeo hero, diagrama de processo, scroll-reveal, formulário de contato, carrossel de clientes, agendamento e o build de deploy — garantindo que futuras alterações não introduzam regressões visuais ou funcionais.

### Estrutura

```
tests/
├── unit/
│   ├── slider.test.js               # cascading slider — lógica isolada
│   ├── build-netlify.test.js        # scripts/build-netlify.js — cache busting (hash md5)
│   ├── clients-carousel.test.js     # carrossel de logos de clientes
│   ├── form.test.js                 # formulário de contato (validação, honeypot, sanitização)
│   └── init-scheduling.test.js      # agendamento/CTA
└── regression/
    ├── slider.regression.test.js         # cascading slider — integridade estrutural
    ├── hero-video.regression.test.js     # vídeo hero (retry, fallback)
    ├── process-diagram.regression.test.js # diagrama de processo
    └── scroll-reveal.regression.test.js  # scroll-reveal via IntersectionObserver (substituiu ScrollTrigger/batchReveal)
```

### O que cada suíte testa

| Suíte | Tipo | Cobertura |
|---|---|---|
| **unit** | Lógica isolada | Proporções do slider, cálculo `getSizes()`, cache busting do build de deploy, validação e sanitização de formulário, carrossel de clientes, agendamento, acessibilidade, assets de imagem |
| **regression** | Integridade estrutural | Escala de imagens, overlay, estados de botões, breakpoints responsivos, camadas z-index, overflow, retry de vídeo, comportamento do scroll-reveal nativo, script integrity |

### Como instalar e executar

```bash
# Instalar dependências (apenas na primeira vez)
npm install

# Executar todos os testes
npm test

# Executar apenas testes unitários
npm run test:unit

# Executar apenas testes de regressão
npm run test:regression
```

### Resultado esperado

```
Test Suites: 9 passed, 9 total
Tests:       119 passed, 119 total
```

### O que os testes garantem

| Regra de Negócio | Teste |
|---|---|
| Proporções do cascading slider por breakpoint (desktop/tablet/mobile/small) | ✅ |
| Navegação circular (loop) e clonagem quando há menos de 9 fotos | ✅ |
| Imagens com escala fixa (sem zoom) | ✅ |
| Overlay transparente (sem escurecimento) | ✅ |
| Botões sem :hover CSS (gerenciado por JS) | ✅ |
| Breakpoints responsivos do slider | ✅ |
| Acessibilidade (ARIA labels) | ✅ |
| Script integrity (funções e constantes do slider) | ✅ |
| Cache busting: `?v=<hash md5>` aplicado a `script.min.js`/`styles.min.css` no build de deploy | ✅ |
| Vídeo hero: retry automático em falha de carregamento | ✅ |
| Scroll-reveal nativo (`IntersectionObserver`) replica o comportamento visual do GSAP/ScrollTrigger original | ✅ |
| Formulário de contato: validação de campos, honeypot anti-bot, sanitização de input | ✅ |
| Carrossel de clientes e agendamento: estrutura e comportamento | ✅ |

---

## ▶️ Como usar

Apenas abra o arquivo `index.html` em qualquer navegador moderno:

```bash
# Windows
start index.html

# macOS
open index.html

# Linux
xdg-open index.html
```

Nenhum servidor, build ou instalação de dependências é necessária.

---

## 📝 Histórico de Mudanças

| Data | Mudança | Arquivos |
|---|---|---|
| 20/08/2026 | **Correção do tremor/flicker do card de contato ("Solicitar Orçamento") no parallax do hero em mobile**, regressão introduzida pela reimplementação anterior de `initHeroContactCardParallax()` que substituiu `ScrollTrigger` por um listener direto no `scroll` do Lenis. Causa raiz: `updateParallax()` recalculava `scrollable = rect.height - window.innerHeight` a cada tick de scroll; `.hero-architectural-scene` usa `height: 100dvh` (confirmado em `styles.css`), que — assim como `window.innerHeight` — oscila em tempo real durante o próprio gesto de scroll no mobile por causa do recolhimento/expansão da barra de endereços, fazendo o `progress` (e o `transform`) pular entre frames. Corrigido cacheando `scrollable` uma única vez na inicialização (`measureScrollable()`), recalculado só em `resize` com mudança real de LARGURA (mesmo guard "largura-only" já usado pelo cascading slider para a mesma classe de bug) — `rect.top` continua lido ao vivo a cada tick, único valor que deve mesmo mudar continuamente durante o scroll. Teste de unidade existente (`hero-contact-parallax.test.js`) ajustado para mockar `getBoundingClientRect()` antes do `require()` (a medição agora acontece na inicialização, não mais a cada chamada do handler); novo teste de regressão cobrindo o guard de resize sem mudança de largura. 123/123 testes passando. Verificado pelo usuário em dispositivo mobile e desktop reais — tremor eliminado | `script.js`, `script.min.js`, `tests/unit/hero-contact-parallax.test.js`, `README.md` |
| 19/08/2026 | **Implementação do lote da auditoria de estrutura/SEO** (inventário completo feito em sessão anterior, sem GSAP/ScrollTrigger neste lote): (1) `robots.txt` — `Sitemap:` apontava para `perinconstrucoes.com.br` (domínio próprio ainda não registrado/propagado), corrigido para `perinconstrucoes.netlify.app` (mesmo domínio já usado no resto do site). (2) `politica-de-privacidade.html` — `<link rel="canonical">` tinha o mesmo problema de domínio, corrigido. (3) Novo teste `tests/unit/build-netlify.test.js` (3 casos) rodando o `scripts/build-netlify.js` real via `child_process` e validando que o cache busting (`?v=<hash md5 de 8 chars>`) aplicado a `script.min.js`/`styles.min.css` no `dist/index.html` bate com o hash real do conteúdo dos arquivos — cobertura que não existia antes (build de deploy não tinha nenhum teste). (4) Seção "🧪 Testes Automatizados" deste README reescrita: estava desatualizada desde antes das últimas 6 suítes serem adicionadas (falava só do cascading slider, "2 passed, 2 total"/"91 passed, 91 total"); atualizada para refletir as 9 suítes e 119 testes reais. (5) `docs/relatorios/RELATORIO-PERFORMANCE.md` — novo capítulo final documentando `0b7703f`/`671ce68`/`232e3af` (ver detalhe na entrada de 18/08/2026 abaixo), incluindo a confirmação de que `0b7703f` foi a implementação real da "direção mais promissora ainda não testada" que o próprio relatório havia apontado. (6) `script.js` (`initCascadingSlider()`) — as 6 fotos do carrossel de portfólio tinham `alt` idêntico ("Obra concluída para Elektro Redes") por herdarem o mesmo `name` em `portfolioProjects`; como não há metadado real de etapa/ângulo/local por foto, diferenciado com índice sequencial factual (`— foto N de 6`) em vez de inventar detalhes; `script.min.js` regenerado via `terser` e revalidado com `check-min-freshness.js`. 119/119 testes passando (nenhum teste cobria o `alt` das fotos antes ou depois, sem markup correspondente) | `robots.txt`, `politica-de-privacidade.html`, `tests/unit/build-netlify.test.js` (novo), `docs/relatorios/RELATORIO-PERFORMANCE.md`, `script.js`, `script.min.js`, `README.md` |
| 18/08/2026 | **Diagnóstico final do item de scroll jank (parallax do hero + contadores) e remoção de `initCounters` como dead code**. Etapa 1 do diagnóstico (medir antes de decidir se valia trocar mecanismo): releitura das duas funções remanescentes revelou que a premissa estava incompleta — `initHeroParallax` não usa `ScrollTrigger` (é só um listener `mousemove`, sem ligação ao scroll) e `initCounters` usa `once:true` (revelação única, mesma categoria já migrada, não scrub contínuo). `ScrollTrigger.getAll().length` real hoje é **1**, não 2: apenas `initHeroContactCardParallax` (`scrub:true` em `.hero-contact-card-parallax`), já confirmado na entrada anterior desta tabela. Não há necessidade de uma segunda rodada de medição de CPU-throttle — o item já foi resolvido pela migração anterior das revelações; 1 instância `scrub:true` isolada não justifica a complexidade de reescrever com listener manual de scroll. Achado à parte durante a releitura: `initCounters()` iterava `.counter-target`, mas esse elemento não existe mais em `index.html` (seção de contadores removida do HTML em algum momento, função órfã ficou rodando sem nenhum efeito a cada carregamento) — função e sua chamada em `runBatchWhenIdle()` removidas. 116/116 testes passando (nenhum teste cobria `initCounters`, sem markup correspondente) | `script.js`, `README.md` |
| 18/08/2026 | **Correção dos 3 achados reais de um diagnóstico de acessibilidade/responsividade (axe-core + medição de bounding rects)**: (1) **15 violações de `color-contrast` (WCAG AA), 2 padrões, ambos design tokens compartilhados**: `--cor-verde-floresta` (`#2A873E`→`#247335`, .section-tag) e `--cor-cinza-acastanhado` (`#857F76`→`#68645C`, labels/aviso de privacidade/footer). Ambos ajustados só na Lightness em HSL, preservando Hue/Saturation (mesma família de cor) — verde de H≈133°/S≈52,5%/L 34,7% para L 29,6% (pior caso medido via axe, `#E1E7D6`, era 3,58:1, agora 4,64:1); cinza de H≈36°/S≈6%/L 49,2% para L 38,4% (pior caso, footer `#EDEDE0`, era 3,36:1, agora ~4,99:1). Achado de processo: a primeira rodada de correção usou o fundo teórico (branco) do `.section-tag` em vez do fundo real composto por seção (`--color-bg-secondary` alternado), subestimando o pior caso — corrigido recalculando contra o valor que o axe mediu de fato. O primeiro valor do cinza (`#6F6A62`, 4,55:1) passava no axe mas com margem curta demais contra o mínimo de 4,5:1; recalculado para `#68645C` (~4,99:1), mesma família HSL, com folga real. 0/15 violações restantes (`axe.run()`, 1440px e 375px). (2) **Menu "Como Trabalhamos" quebrando em 2 linhas em 769-1023px**: nova regra `@media (min-width:769px) and (max-width:1023px)` reduzindo `gap`/`font-size`/padding do nav nessa faixa específica, sem tocar nos breakpoints de 768px (hamburger) nem 1024px (nav desktop full) — confirmado por bounding rect (`getClientRects().length === 1` em 769/820/900/1023px) e screenshot idêntico nos dois limites. (3) **Footer sobreposto pelo botão flutuante do WhatsApp em 769-900px**: `padding-bottom: 120px` em `.premium-footer` nessa faixa, espaço suficiente para o botão fixed (56px + 24px de offset) não cobrir `.footer-bottom` ao rolar até o fim — confirmado sem sobreposição via medição de bounding rect (`overlaps()`) em 769/800/850/900px. Bug de processo descoberto durante a verificação (mesma classe já documentada nas linhas abaixo): `styles.css` editado mas `styles.min.css` (bundle real carregado por `index.html`) não regenerado — corrigido via `npx clean-css-cli` + `node scripts/check-min-freshness.js`. 116/116 testes passando, `npm run build:netlify` OK | `styles.css`, `styles.min.css` |
| 18/08/2026 | **Substituição das ~26-27 instâncias de `ScrollTrigger`/`ScrollTrigger.batch()` (revelação em scroll de `.differential-item`, `.service-mosaic-item`, `.process-circle-cluster`/`.process-accordion-item`) por `IntersectionObserver` nativo + CSS transition, mantida e aplicada após atender ao critério de decisão** (quarta tentativa de otimização de scroll desta série — as 3 anteriores, lazy-creation em duas variantes e fatiamento do Grupo A, foram descartadas por regressão ou resultado misto, ver entradas anteriores). Implementação: cada elemento das 3 categorias com efeito visual real ganha seu próprio `IntersectionObserver` (`rootMargin` replicando os thresholds do GSAP original), aplicando/removendo `.is-revealed`; transição via CSS (`opacity`/`translate`, `0.6s cubic-bezier(0.215, 0.61, 0.355, 1)`, equivalente a `power2.out`) com stagger por `transitionDelay` calculado em JS. Comportamento assimétrico do GSAP original preservado: item some ao rolar para cima e sair por baixo (`onLeaveBack`), mas não some ao rolar para baixo e sair por cima (sem `onLeave` vinculado no original). `.section-title-reveal`/`.text-reveal` (sem efeito visual algum previamente) mantidos como no-op, sem observer. **Bug crítico encontrado e corrigido durante a verificação**: `script.min.js`/`styles.min.css` (bundles carregados por `index.html` em produção, não os fontes) não haviam sido regenerados após a edição de `script.js`/`styles.css` — o site servido continuava rodando o código antigo (`ScrollTrigger.getAll().length` retornava 26 no browser ao vivo, contradizendo o código-fonte já alterado); corrigido regenerando via `terser`/`clean-css-cli` e revalidado com `node scripts/check-min-freshness.js` (após o fix, `ScrollTrigger.getAll().length` caiu para 1 — resta apenas o parallax não relacionado da hero). Verificação visual das 3 categorias via browser (estado inicial oculto, revelação em stagger no scroll rápido e lento, esconder assimétrico) sem regressão. Medição de performance com CPU throttle 6x, scroll de página inteira, 3 rounds mobile (390×844) + 2 rounds desktop (1440×900), comparando contra baseline via `git worktree` do HEAD (mesma metodologia ad-hoc, script Puppeteer não versionado): contagem de "peaks" (frame gaps > 50ms) caiu de forma consistente nos 3 rounds mobile (18→10, 14→9, 12→8) e também no desktop (17→14, 13→12), sem regressão em nenhuma métrica; FPS médio ficou estável/levemente melhor, dentro do ruído esperado de medição headless. Atendeu ao critério de decisão definido nas tentativas anteriores (melhora consistente nos 3 rounds mobile, sem piora em nenhuma métrica) — mudança mantida. 116/116 testes passando (10 novos de regressão em `tests/regression/scroll-reveal.regression.test.js`) | `script.js`, `script.min.js`, `styles.css`, `styles.min.css`, `tests/regression/scroll-reveal.regression.test.js` (novo) |
| 18/08/2026 | **Terceira tentativa de mitigar o custo de `ScrollTrigger.refresh()` durante o scroll — criação lazy via `IntersectionObserver` — testada e descartada** (mesmo padrão de duas tentativas anteriores já descartadas: fatiar o Grupo A em grupos menores, com e sem espaçamento). Implementação: `lazyCreateForSection()` (modelada em `scheduleClientsCarouselEarly()` já existente) adiava a criação do `ScrollTrigger` de cada seção (`initScrollReveals()` escopado por `root`, mais `initServicesReveal`/`initProcessDiagram`/`initDifferentialsAnimation`) até a seção se aproximar da viewport (`rootMargin:'400px 0px'`), mantendo o gate original do hero como primeira camada. Suíte completa validada (112 testes, incluindo 6 novos cobrindo scroll rápido/fora de ordem e resize com triggers parcialmente criados) antes da medição. Medição com CPU throttle 6x, scroll da página inteira, 3 rounds mobile (390×844) + 2 rounds desktop (1440×900), comparando contra baseline do código original medido com a mesma metodologia (script Puppeteer ad-hoc, não versionado): mobile teve resultado misto (peaks melhoraram em 2 de 3 rounds mas FPS ficou levemente abaixo do baseline nos 3 rounds, sem nunca superá-lo); desktop teve os peaks praticamente dobrando de forma consistente nos 2 rounds (13/12 → 20/22) — regressão real, não ruído de medição. Não atendeu ao critério de decisão definido previamente (melhora consistente nos 3 rounds mobile, sem piora em nenhuma métrica) — revertido via `git checkout -- script.js`, teste `tests/unit/lazy-scroll-reveal.test.js` removido. 106/106 testes passando (estado idêntico ao anterior à tentativa) | nenhum (revertido) |
| 17/08/2026 | **Revertida a otimização de CSS crítico inline + `defer-css.js`** (introduzida em rodada anterior de perf, não documentada nesta tabela): em produção com cache limpo, tudo a partir da seção "sobre nós" renderizava sem estilo por 2-3 segundos — exatamente a fronteira onde o CSS crítico inlinado (gerado só para o primeiro viewport, 375×812 + 1440×900) termina. Sintomas confirmados pelo usuário: seções sem formatação aparecendo tarde, carrossel automático com tamanho errado no mobile, banner do portfólio com layout quebrado. O ganho de Lighthouse que motivou a otimização (score mobile 66→81) media o tempo até o primeiro paint, não o fato de a página parecer quebrada durante o carregamento — troca ruim para um site institucional. Removido o bloco `<style id="critical-css">` e o `<script src="defer-css.js">`/`<noscript>` do `<head>` de `index.html`, restaurado o `<link rel="stylesheet" href="styles.min.css">` bloqueante simples no lugar; arquivo `defer-css.js` deletado e removido da lista `FILES` de `scripts/build-netlify.js`. Confirmado que a correção de `blur(8px)` do WebKit (existia em três cópias, uma delas o CSS crítico inline removido aqui) permanece intacta em `styles.css`/`styles.min.css`. 103/103 testes passando | `index.html`, `scripts/build-netlify.js`, `defer-css.js` (removido) |
| 14/08/2026 | **Instrumentação temporária de isolamento (`?isolate=...`) para investigar travamento reportado ao aparecer o texto de entrada da hero, especificamente em Safari iOS mobile** (recorrência suspeita da classe de bug do `RELATORIO-PERFORMANCE.md`, seção "Travamento recorrente no Safari iOS", já confirmada sem regressão do fix original — `blur(24px)` intacto). Recriado o mecanismo de flags de eliminação usado em julho/2026 (removido do código após aquela investigação), atualizado para os candidatos desta rodada: `no-particles` (desliga `createParticles()`), `no-gsap` (pula a timeline GSAP de `initHeroEntrance()`, aplica estado final direto), `no-geometries` (remove do DOM grid/formas/luzes do hero), `no-blur-only` (zera `filter` dos `.light-spot`, mantendo-os visíveis, para descartar de vez reincidência mascarada do blur já corrigido), `no-defer-css` (novo candidato: impede `defer-css.js` de carregar `styles.min.css`, cujo fallback sem `requestIdleCallback` no Safari cai em `window.load` — mesma classe de bug já vista na fila de partículas) e `all` (liga todas). Instrumentação existente `?debug=perf` confirmada funcional com o código atual (hooks em `initHeroEntrance`/`runBatchWhenIdle`/`initClientsCarousel` inalterados), exibindo resultado via overlay na própria tela (botão "Copiar log de performance", sem necessidade de Mac/inspeção remota). Como o teste real acontece em `http://<IP>:3000` (HTTP puro, sem TLS), `navigator.clipboard` não existe em contexto inseguro no Safari — o botão de copiar já cobria esse caso (`else` quando a API está ausente), mas faltava cobrir o caso em que a API existe porém `writeText()` rejeita a Promise (`.catch()`); ambos os caminhos agora caem no mesmo fallback `showFallbackTextarea()`, que exibe um overlay com `<textarea readonly>` pré-selecionado para copiar manualmente por toque longo. Testado forçando a rejeição via `Object.defineProperty(navigator, 'clipboard', ...)` no console — confirmado visualmente que o botão muda para "❌ Falhou — copie abaixo" e o textarea de fallback aparece. Adicionado também um modo `?debug=perf&autocycle=1` que testa as 6 condições de isolamento em sequência automaticamente (reload entre cada uma, progresso salvo em `sessionStorage`), com indicador de progresso na tela e overlay final combinando os 6 logs rotulados — evita repetir manualmente os 6 testes no iPhone a cada rodada. Testado localmente (servidor estático na raiz do projeto) simulando o ciclo completo: confirmado que o `sessionStorage` acumula as 6 condições corretamente através dos reloads, sem perda, e que o overlay final combina os 6 logs rotulados corretamente. Regenerado `script.min.js` via `terser`; `check-min-freshness.js` e os 103 testes (6 suítes) passando; confirmado via `dist/` que a instrumentação sobrevive à minificação e ao `build:netlify`. Nada disso deve ir para produção como está — é debug temporário para o teste no iPhone do usuário final, a remover após o diagnóstico | `script.js`, `script.min.js`, `defer-css.js`, `README.md` |
| 12/08/2026 | **Endurecimento de segurança e conformidade LGPD do site**, quatro itens da auditoria de segurança anterior: (1) **Security headers**: novo bloco `/*` no topo do `_headers` (mantidos os blocos de `Cache-Control` existentes) com CSP (`script-src 'self'` sem `unsafe-inline` — confirmado que não há `<script>` inline nem atributos `onclick=`/`onload=` no site; `style-src 'self' 'unsafe-inline'` mantido por dois atributos `style=""` estáticos legítimos, no wrapper do honeypot e no iframe do Google Maps), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` desabilitando geolocation/camera/microphone/payment e `Strict-Transport-Security`. (2) **Aviso de privacidade LGPD no formulário de contato**: texto de consentimento abaixo do botão de envio linkando para nova página `politica-de-privacidade.html` (dados coletados, finalidade, sem compartilhamento com terceiros, canal de solicitação de exclusão via WhatsApp), com link também no rodapé do site; página adicionada à lista `FILES` de `scripts/build-netlify.js` para entrar no `dist/`. (3) **`maxlength` nativo nos campos do formulário**: `formName` 100, `formEmail` 180, `formMessage` 1000 — mesmos limites já aplicados no lado JS por `sanitizeString()`, sem mudança de comportamento, só limite visível/nativo. (4) **`npm audit fix` (sem `--force`)**: reduziu vulnerabilidades de 23 (0 crítica, 6 alta, 17 moderada) para 3, todas altas, confinadas a `sharp-cli`/`sharp` (devDependency usada só localmente para otimização de imagem, não roda em produção); resolver as 3 restantes exigiria `npm audit fix --force` (downgrade breaking para `sharp-cli@4.2.0`) — decisão do usuário foi não forçar agora dado o baixo risco real, revisitar futuramente testando a versão nova numa branch separada antes de migrar. 95/95 testes passando (sem alteração no total de testes nesta rodada — os 95 já eram o baseline antes destas mudanças) | `_headers`, `index.html`, `styles.css`, `styles.min.css`, `politica-de-privacidade.html` (novo), `scripts/build-netlify.js`, `package.json`, `package-lock.json` |
| 11/08/2026 | **Adicionadas 5 fotos reais da obra Elektro Redes ao carrossel de portfólio**: usuário colocou 5 fotos (`concluido02` a `concluido06.jpeg`) numa pasta de staging (`assets/imagens carrossel manual/`, não versionada); fotos identificadas visualmente como do projeto "Elektro Redes" (equipamentos de subestação, boxes de sucata de cabo, torres de transmissão). Convertidas para `.webp` (qualidade 80, via `sharp`, mesma resolução 1280×960 dos placeholders existentes) e movidas para `assets/images/portfolio/elektro-02.webp` a `elektro-06.webp`; pasta de staging removida. Adicionados 5 novos slides ao array `portfolioProjects` (todos `name: 'Elektro Redes'`, mesma `subtitle`), mantendo o slide original (`id: 'elektro'`, cover `placeholder-obra-03.webp`) intocado, a pedido do usuário — carrossel de portfólio passou de 6 para 11 slides. Teste que fixava a contagem exata em 6 (`portfolioProjects data array exists with 6 projects, one cover photo each`) foi relaxado para `toBeGreaterThanOrEqual(6)`, já que os demais testes de slide count já usavam essa asserção. 94/94 testes passando | `script.js`, `script.min.js`, `tests/unit/slider.test.js`, `assets/images/portfolio/elektro-02.webp` a `elektro-06.webp` (novos) |
| 11/08/2026 | **Causa raiz real do logo ISA Energia pálido no desktop, encontrada e corrigida** (a correção anterior de `opacity:1` nesta mesma tabela ajudou mas não resolveu — usuário confirmou que o problema persistia): medição por amostragem de pixels (canvas no Chrome e `sharp` via Node) provou que não é um problema de matiz/hue nem só de opacidade do card — é a **rasterização vetorial ao vivo do SVG fonte** (`isa-energia-logo-otimizado.svg`, ~775 paths minúsculos com `clipPath` simulando um degradê) perdendo cobertura de alpha em baixo DPR: renderizado direto a ~119×68px (desktop 1x) o alpha médio ficou em 94-140/255; ao mesmo SVG a ~357×204px (mobile 3x) subiu para 200-214/255 — mesma cor (RGB idêntico), só que com muito menos "solidez" nos traços finos por falta de pixels para antialiasing, lido visualmente como pálido. Corrigido pré-rasterizando o SVG uma única vez, na resolução nativa do `viewBox` (1200×685, via `npx sharp -i isa-energia-logo-otimizado.svg -o isa-energia-logo.png resize 1200 685`), e trocando o `<img src>` do carrossel para esse PNG — deixar o navegador reamostrar um bitmap já com cobertura de alpha completa elimina a dependência de DPR (alpha médio a 119×68 subiu de 140 para 163 usando o bitmap como fonte, medido com `sharp`). Mantido `filter: saturate(2) contrast(1.3)` e `opacity: 1` (correção anterior) para o resultado final. Validado visualmente via Chrome com servidor local: cor sólida e vívida em estado padrão (sem hover), igual ao que já aparecia no mobile. 91/94 testes passando (3 falhas pré-existentes e não relacionadas, dos `.webp` já removidos) | `index.html`, `styles.css`, `styles.min.css`, `assets/images/clients/isa-energia-logo.png` (novo) |
| 11/08/2026 | **Correção dos 3 testes que falhavam desde o início da sessão** (`eldorado.webp exists`, `elektro.webp exists`, `project cover images exist`): causa raiz era `portfolioProjects` (dados do carrossel de portfólio, não do carrossel de clientes) referenciando `assets/images/clients/eldorado.webp`/`elektro.webp`/`isa-energia.webp` como foto de capa dos projetos "Eldorado Brasil", "Elektro Redes" e "ISA Energia" — arquivos que o usuário havia apagado no início da sessão (o logo do carrossel de clientes já havia migrado para SVG/PNG, mas essas 3 fotos de capa do portfólio nunca tinham outra fonte). Corrigido apontando as 3 capas para fotos reais de obra já disponíveis e não utilizadas em `assets/images/placeholders/` (`placeholder-obra-02/03/04.webp`; a `-01` já era usada por "Residencial Villaggio"), validado visualmente via Chrome (servidor local) — as 3 fotos aparecem no carrossel do portfólio, sem imagem quebrada. Testes de unidade e regressão que verificavam a existência/uso desses `.webp` de clientes apagados foram atualizados para refletir os novos caminhos; os 2 testes que checavam `eldorado.webp`/`elektro.webp` como logo do carrossel de clientes (já obsoletos desde a migração para SVG/PNG) foram substituídos por checagens de `elektro.svg`/`isa-energia-logo.png`. 94/94 testes passando (0 falhas) | `script.js`, `script.min.js`, `tests/unit/slider.test.js`, `tests/regression/slider.regression.test.js` |
| 11/08/2026 | **Ajuste fino de intensidade do logo ISA Energia**: com a cobertura de alpha corrigida pelo PNG pré-rasterizado (linha acima), o usuário reportou que o `filter: saturate(2) contrast(1.3)` — mantido da correção anterior como compensação do problema de alpha do SVG, que já não existe mais — deixou a cor forte/exagerada demais. Removido o `filter` por completo; a cor natural do PNG (sem nenhuma manipulação) já corresponde ao tom correto visto no mobile. Mantido apenas `opacity: 1` | `styles.css`, `styles.min.css` |
| 11/08/2026 | **Correção do carrossel de clientes "congelado" no load/F5**: `initClientsCarousel()` só rodava depois que a timeline de entrada do hero terminava (~1.5s-1.8s, proposital, para não competir por CPU com a animação do hero) — se o usuário rolasse até a seção de clientes antes disso, via um carrossel parado ("congelado") até o init tardio rodar ("volta"). Adicionado `scheduleClientsCarouselEarly()`: observa `#clientsStage` com `rootMargin:'800px'` e, assim que a seção se aproxima da viewport, inicializa o carrossel de imediato via `requestIdleCallback` — sem esperar o hero terminar. Guard `clientsCarouselInitialized`/`initClientsCarouselOnce()` garante que o init tardio original (`startIdleQueue`, ainda mantido como fallback) não rode duas vezes. Não altera a proteção original de CPU durante a animação do hero (o `IntersectionObserver.observe()` inicial é barato, sem tocar em layout/DOM) | `script.js` |
| 11/08/2026 | **Correção de dois bugs no autoplay/drag do carrossel de clientes** (`initClientsCarousel()`), causando o sintoma "trava e volta" reportado pelo usuário: (1) `setWidth`/`slideWidth` (usados no salto do loop infinito) eram calculados só com `offsetWidth`, ignorando o `gap` do flex (`16px`/`12px`/`10px` por breakpoint) — o salto de wrap (`currentX -= setWidth`) ficava sistematicamente errado por `totalOriginal * gap` px a cada ciclo, visível como um salto brusco periódico durante o autoplay; corrigido lendo o `gap` real via `getComputedStyle` (`getGap()`) e incluindo-o no cálculo, com releitura no `ResizeObserver` para acompanhar mudanças de breakpoint. (2) o loop `animate()` incrementava `currentX` a cada frame mesmo com `isDragging=true`, competindo com `onPointerMove` (que também escreve `currentX` a partir do gesto do usuário) — a soma das duas fontes causava o travamento/correção brusca ao soltar o arraste; corrigido movendo o incremento/wrap de `currentX` para dentro do bloco `if (!isDragging)`, deixando `onPointerMove` como única fonte durante o drag | `script.js` |
| 10/08/2026 | Substituição do logo Eldorado no carrossel de clientes: trocado o `<img>` raster (`assets/images/clients/eldorado.webp`) por SVG inline (wordmark + símbolo com gradiente), a pedido do usuário. Mantida a classe `.clients-carousel-img` (mesmo `object-fit:contain`/padding responsivo já existente) e paridade de acessibilidade via `role="img" aria-label="Logo Eldorado"` no lugar do `alt`. IDs dos `<linearGradient>` renomeados de `a`/`b`/`c` (genéricos, do SVG de origem) para `eldoradoGradA/B/C` para evitar colisão com outros SVGs inline da página | `index.html` |
| 10/08/2026 | **Restrição do escopo de deploy do Netlify**: adicionado `netlify.toml` (`build.command` + `publish = "dist"`) e script `scripts/build-netlify.js` que monta `dist/` copiando apenas os arquivos/pastas de produção (`index.html`, `styles.min.css`, `script.min.js`, `_headers`, `robots.txt`, `sitemap.xml`, `fonts/`, `vendor/`, `assets/`). Antes, o Netlify publicava a raiz do repo sem filtro, expondo publicamente `audit/`, `legado/`, `prototipos/`, `assets-fonte/`, `tests/`, `scripts/`, `docs/` via URL direta. Nenhum conteúdo de pasta foi alterado/removido do repositório — apenas o que vai para o deploy público. | `netlify.toml`, `scripts/build-netlify.js`, `package.json`, `.gitignore`, `README.md` |
| 10/08/2026 | **Arquivamento de material de dev/histórico** para fora do fluxo ativo do repositório, via `git mv` (histórico preservado): `assets-fonte/` → `_archive/assets-fonte/`, `legado/` → `_archive/legado/`, `prototipos/extracao-perin/` → `_archive/prototipos-extracao-perin/`. `prototipos/cascading-slider/` NÃO foi movido — continua em `prototipos/` por ser referência oficial documentada em `CLAUDE.md`. Removido do controle de versão (`git rm`) o arquivo `assets/images/about/Perin_Rev-.code-workspace` (lixo de editor sem função no projeto); adicionado `*.code-workspace` ao `.gitignore` para prevenir recorrência. Atualizados 5 comentários em `index.html`/`styles.css`/`script.js` que referenciavam o caminho antigo `extracao-perin/...` para o novo `_archive/prototipos-extracao-perin/...` (comentários apenas — nenhuma referência funcional/`src`/`url()` apontava para essas pastas). 94 testes passando | `.gitignore`, `README.md`, `index.html`, `styles.css`, `script.js`, `_archive/assets-fonte/`, `_archive/legado/`, `_archive/prototipos-extracao-perin/` |
| 02/08/2026 | Ajuste do efeito de "vidro fosco" na foto da seção "Sobre" (`.about-image-frame`) a pedido do usuário: `.about-image-overlay` passou a ser um escurecimento fixo e permanente (`rgba(10,10,10,0.3)`, sem `backdrop-filter`/blur — só escurece, não embaça), igual em estado normal e hover, para não mudar a cor da imagem ao passar o mouse. O hover mantém apenas o zoom leve (`scale(1.03)`, transição 0.6s) já existente na imagem e ganha um brilho suave ao redor do frame via `box-shadow` (glow em preto/cinza, `rgba(0,0,0,...)` — trocado do verde da marca a pedido do usuário, que achou o tom verde ruim; opacidade reduzida de 0.45/0.25 para 0.22/0.12 a pedido do usuário para deixar mais leve, transição 0.6s) | `styles.css` |
| 30/07/2026 | Adição do botão flutuante fixo de WhatsApp (`.whatsapp-float-btn`): ícone SVG inline (sem dependência de fonte de ícone externa), `position:fixed` no canto inferior direito (24px desktop, 16px em ≤480px), fundo `#25D366`, formato circular 56×56px (52×52px mobile, acima do mínimo de toque de 48px), animação de pulso sutil via `@keyframes` (pausada no hover, desativada com `prefers-reduced-motion:reduce`), hover com `scale(1.08)` e sombra mais forte. `z-index:990`, abaixo do overlay do menu mobile (`z-index:999`) para nunca cobri-lo quando aberto. Link `https://wa.me/5518997375322` sem parâmetro `?text=` (a pedido do usuário: apenas o ícone, sem mensagem pré-preenchida), `target="_blank" rel="noopener noreferrer"` e `aria-label="Fale conosco no WhatsApp"`. Validado visualmente em desktop via browser local (fixed durante scroll, hover, cor, raio, ausência de `?text=` no link) e via inspeção computada de CSS (z-index/posicionamento). 94 testes passando | `index.html`, `styles.css`, `styles.min.css`, `README.md` |
| 30/07/2026 | Ajuste do fundo de `.process-journey-section` para `var(--color-bg-secondary)`, alinhado ao mesmo tom usado em `.faq-section`. Removida por completo a seção "Números" (placeholder aguardando dados reais do cliente, id `#numbers`), incluindo markup HTML, regra `#numbers` de alternância de cores entre seções e as duas variantes mobile de `.numbers-grid`/`.stat-card` — sem uso pendente em `script.js` ou navegação. Teste de regressão de `.process-journey-section` atualizado para o novo token. 94 testes passando | `index.html`, `styles.css`, `styles.min.css`, `README.md`, `tests/regression/process-diagram.regression.test.js` |
| 30/07/2026 | **Simplificação da interação do diagrama "Como Trabalhamos" para clique universal** e **correção de bug de fundo/texto brancos**: (1) removida a distinção por breakpoint (`isClusterVisible()`) em `initProcessDiagram()` — clique (não mais hover) ativa a etapa tanto no círculo quanto no item do accordion, em qualquer largura de tela; removida também a regra CSS que restaurava `cursor:default` no desktop, já que `cursor:pointer` agora vale universalmente. (2) Corrigido bug onde a seção `#process` renderizava com fundo e texto claros, tornando o conteúdo ilegível: causa raiz era a regra `#process { background-color: var(--color-bg-secondary) }` no bloco de alternância de cores entre seções, que sobrescrevia por especificidade de ID o tema escuro próprio definido em `.process-journey-section` (mesmo elemento, ambos atributos). Removida a regra e documentada a exceção junto às demais (Hero, CTA Principal). Validado visualmente em browser. 87 testes passando (regra de regressão nova cobrindo clique-only e o bug de fundo) | `script.js`, `styles.css`, `script.min.js`, `styles.min.css`, `tests/regression/process-diagram.regression.test.js` |
| 29/07/2026 | **Implementação do JS do diagrama circular "Como Trabalhamos"** (`initProcessDiagram()`): sincroniza o cluster de 4 círculos (topo/direita/baixo/esquerda) com o accordion de etapas. Desktop (>1024px): hover em qualquer item do accordion (ou no círculo) ativa os dois em conjunto — apenas um step ativo por vez, com o círculo exibindo o crossfade "/01"-"/04" e a animação de radar (2 arcos concêntricos) rodando somente no círculo ativo; o item ativo do accordion expande via `grid-template-rows`. Mobile/tablet (≤1024px, cluster oculto por CSS): sem hover (touch), o clique no head do accordion expande/colapsa, independente do estado de hover. Validado visualmente em browser (hover por item, troca rápida entre steps, clique isolado no breakpoint mobile) antes da escrita dos testes. Corrigido também `cursor` do `.process-accordion-head` (era `default` sempre; agora `pointer` em ≤1024px e `default` em desktop, onde o hover já controla o estado). 84 testes passando (4 novos de regressão) | `script.js`, `styles.css`, `script.min.js`, `styles.min.css`, `tests/regression/process-diagram.regression.test.js` |
| 29/07/2026 | Correção do espaçamento vertical do `.cta-principal-container`: a centralização com `flex/align-items:center` (introduzida na alteração anterior de 100vh) empurrou o bloco de texto para baixo, separando-o do "VERDADE" de fundo. Revertido para o padding top-anchored original (`170px 64px 100px` desktop, `115px 48px 70px` tablet, `118px 28px 32px` mobile), preservando a sobreposição de ~metade do "VERDADE" atrás do badge/título como no design aprovado, mantendo `min-height:100vh` na seção | `styles.css`, `styles.min.css` |
| 29/07/2026 | Aumento do botão `.cta-principal-btn` no desktop (`min-height:56px; padding:16px 36px; font-size:16px`) e conversão da `.cta-principal-secao` para `min-height:100vh` com centralização vertical (`display:flex; align-items:center`) em todos os breakpoints, seguindo o padrão moderno de seções principais ocupando ~100% da viewport — aplicado apenas nesta seção (principal), não em seções secundárias | `styles.css`, `styles.min.css` |
| 29/07/2026 | Correção do tamanho dos botões `.cta-principal-btn` (estavam minúsculos: 13px/8px 15px). O `.dc.html` do export continha um valor de override experimental; a captura de tela do usuário (10 min mais recente, portanto mais autoritativa) e o componente base `.pds-button` do design system confirmam tamanho padrão de pill button: `min-height:48px; padding:14px 32px; font-size:15px`. Aplicado em todos os breakpoints (mobile com padding/fonte levemente reduzidos: `13px 24px`/`14px`) | `styles.css`, `styles.min.css` |
| 29/07/2026 | Correção do `max-width` do `.cta-principal-container` (600px→728px desktop, 480px→576px tablet): o valor copiado do export de design assumia `box-sizing: content-box`, mas o projeto usa `border-box` global, o que estreitava o conteúdo em 128px/96px e quebrava o título em 4 linhas em vez de 2. Confirmado visualmente contra a captura de tela de referência do usuário | `styles.css`, `styles.min.css` |
| 11/08/2026 | Correção do logo ISA Energia no carrossel de clientes: o SVG fonte (`isa-energia-logo-otimizado.svg`) é resultado de uma exportação que converteu um gradiente vetorial contínuo em ~500 paths de cor sólida em degradê (stepped-gradient), o que faz o traço fino do wordmark cursivo renderizar visivelmente pálido/dessaturado em relação aos demais logos, confirmado via renderização real (Playwright/Chromium) reproduzindo o CSS exato do carrossel. Como não há fonte vetorial limpa alternativa, a correção foi aplicada via `filter: saturate(2) contrast(1.3)` escopado a `.clients-carousel-img[alt="Logo ISA Energia"]`, sem alterar o asset original. Também foi vinculada a logo da Elektro (renomeada de `Logomarca_Neoenergia_Elektro (1).svg` para `elektro.svg`, sem espaços/parênteses) no lugar da referência quebrada a `elektro.webp` (deletado) | `index.html`, `styles.css`, `assets/images/clients/elektro.svg` (renomeado) |
| 11/08/2026 | Correção de follow-up: a alteração anterior em `styles.css` não estava surtindo efeito no site real porque `index.html` carrega `styles.min.css` (não `styles.css`) — regenerado via `npx clean-css-cli styles.css -o styles.min.css` e revalidado com `node scripts/check-min-freshness.js`. Também investigado o comportamento do carrossel automático (`initClientsCarousel()` em `script.js`): o loop infinito clona os slides via `cloneNode(true)`, que preserva `class`/`alt`, então o seletor CSS por atributo continua válido nos clones — não é fonte do problema. Confirmado visualmente via Playwright/Chromium contra a página real (não um snapshot estático) que só o logo ISA foi afetado pelo filtro; os demais logos (Elektro, Eldorado, State Grid) permanecem inalterados | `styles.min.css` |
| 09/06/2026 | Criação inicial do projeto | HTML, CSS, JS |
| 09/06/2026 | Substituição da paleta de cores | `styles.css` |
| 09/06/2026 | Renomeação para kebab-case | HTML, CSS, JS |
| 09/06/2026 | Remoção do Lenis (scroll conflitante) | `index.html`, `script.js` |
| 09/06/2026 | Substituição do Swiper por Cascading Slider | HTML, CSS, JS |
| 09/06/2026 | Ajustes de layout (gap, altura, largura) | `script.js` |
| 09/06/2026 | Transição mais lenta (1.2s) | `script.js` |
| 09/06/2026 | Texto apenas no slide central com fade | HTML, CSS, JS |
| 09/06/2026 | Delay do texto reduzido (0.7s → 0.4s) | `script.js` |
| 09/06/2026 | Documentação atualizada | `README.md` |
| 10/06/2026 | Substituição do select nativo por dropdown customizado no formulário de contato, com ícones SVG por tipo de serviço, estilos com a paleta verde do site e navegação por teclado | `index.html`, `styles.css`, `script.js` |
| 25/06/2026 | Ajustes no carrossel automático de clientes: cards ~13% maiores (215×152px), proporção menos alongada (~1.41:1), velocidade do autoplay aumentada (2.8 px/frame), inércia mais forte (FRICTION 0.95, DRAG_FACTOR 1.05, MAX_SPEED 22), direção inicial ← ← ← (direita para esquerda, baseSpeed=-2.8), definição JS das variáveis CSS `--slide-w`/`--slide-h` | `styles.css`, `script.js` |
| 30/06/2026 | Reformulação completa da seção Portfólio: nova arquitetura dual Galeria + Visualizador com transição contínua. Galeria com grid responsivo 3/2/1 colunas, cards padronizados 16:10 com hover elegante (overlay gradiente, botão "Ver Projeto" animado). Carrossel premium extraído para engine reutilizável `createCascadingSlider()`. Transição cinematográfica: expansão contínua do card clicado até ocupar área do visualizador, foto de capa se transforma no primeiro slide sem trocar de imagem. Retorno com animação reversa e restauração exata do scroll. Dados dos projetos movidos para array `portfolioProjects` com 6 projetos. Testes atualizados para nova arquitetura (107 testes passando) | `index.html`, `styles.css`, `script.js`, `tests/unit/slider.test.js`, `tests/regression/slider.regression.test.js` |
| 30/06/2026 | Reimplementação da transição como Shared Element Transition: eliminados clones (`expandCard`/`shrinkCard`), o card é o protagonista único durante toda a animação. Timeline única orquestra todas as fases — outros cards perdem destaque gradualmente, card expande continuamente, slides nascem ao redor, títulos e setas aparecem sequencialmente. Fechamento é o reverso exato da abertura. Viewer nunca usa `display:none` — transição por `opacity`/`pointer-events`. Nenhum frame vazio entre estados. 107 testes passando | `script.js`, `styles.css` |
| 30/06/2026 | Refinamento da animação para continuidade orgânica: easing unificado (`cubic-bezier(0.55, 0, 0.15, 1)`) com início suave e desaceleração fluida. Todas as fases se sobrepõem naturalmente — nada começa após o término da anterior. Outros cards e galeria compartilham o mesmo timing do movimento mestre (expansão/contração do card). Fechamento implementado como espelho exato da abertura. Timeline de abertura: ~0.76s. Timeline de fechamento: ~0.62s. 107 testes passando | `script.js` |
| 30/06/2026 | Reimplementação conceitual completa da transição como Shared Element Transformation: slider pré-populado e pré-posicionado invisível antes da animação começar. Card é o protagonista perpétuo — nunca desaparece durante a expansão. Slides nascem ao redor do card a 70% da expansão. Carrossel já está montado e posicionado desde o frame 0, apenas invisível. Easing físico: `cubic-bezier(0.25, 0.10, 0.10, 1.00)` — início muito lento, meio acelerado, desaceleração longa e suave. Fechamento literalmente o vídeo ao contrário. Nenhum `display:none` durante a animação. Card só é escondido após slides visíveis (merge natural). 107 testes passando | `script.js` |
| 30/06/2026 | Transição premium nível Awwwards: master curve única `cubic-bezier(0.25, 0.46, 0.45, 0.94)` para todos os movimentos. Card nunca é substituído — o slide 0 do carrossel É o próprio card (placeholder crossfade). Slides laterais entram com `blur(4px)→0` + `opacity`, sensação cinematográfica. Outros cards recuam com stagger por distância. Fechamento com destruição em ordem reversa: botões → títulos → slides com blur → card reemerge → shrink → galeria retorna com stagger por proximidade. Curva idêntica em ambas direções. 107 testes passando | `script.js` |
| 01/07/2026 | Refatoração completa da arquitetura do portfólio: criado `.portfolio-stage` como palco único com `position:relative`. Gallery e viewer são camadas sobrepostas (`position:absolute; inset:0`). 107 testes passando | `index.html`, `styles.css`, `script.js` |
| 01/07/2026 | Transição surface-slide (referência eduardbodak.com): viewer desliza de baixo como nova superfície (`translateY 100%→0`, 800ms, `cubic-bezier(0.25,0.46,0.45,0.94)`). Conteúdo interno aparece sequencialmente só após 80% da superfície posicionada: título (-12px→0, 200ms) → subtítulo (+60ms) → botão voltar (+120ms, fade) → setas carrossel (+150ms, scale 0.95→1) → slides laterais (+180ms, translateX ±30→0) → conteúdo slide ativo (+220ms, fade leve). Fechamento é reverso exato: conteúdo desaparece → superfície desce (0→100%). Gallery fade atrás. Carrossel e GSAP do slider não foram alterados. 107 testes passando | `script.js` |
| 01/07/2026 | Correção de bugs da transição surface-slide: viewer invisível (opacity nunca setada para 1) e falta de profundidade (galeria sumia em vez de escurecer atrás). Adicionado overlay `.portfolio-gallery-dim` interno à galeria com animação de `transparent` → `rgba(0,0,0,0.30)` durante a subida do viewer. CSS `.staged` não zera mais opacity/visibility da galeria — ela permanece visível atrás com o overlay de escurecimento. Timeline openProject garante estado final: `gsap.set(viewer, { opacity:1, visibility:visible, y:'0%', pointerEvents:'auto' })` no `onComplete`. Gallery só some (visibility:hidden) após animação completa. Fechamento: overlay clareia de volta, galeria reaparece atrás do viewer durante a descida. z-index do viewer ajustado para 3 (acima do dim z-index:2). 107 testes passando | `index.html`, `styles.css`, `script.js` |
| 01/07/2026 | Transição "curtain" (cortina editorial): substitui surface-slide. Nova abordagem conceitual — uma cortina com fundo bege (`var(--color-bg-secondary)`) cobre toda a seção (`y:100%→0%`, 550ms, `cubic-bezier(0.55,0.06,0.35,0.94)`) com leve escurecimento (`rgba(0,0,0,0)→rgba(0,0,0,0.22)`). No ponto médio (totalmente coberto), swap silencioso via `tl.call()`: gallery desmonta e viewer monta atrás da cortina. Depois cortina revela (`y:0%→-100%`, 550ms) enquanto escurecimento reduz. Conteúdo do viewer (título, subtítulo, botão voltar, setas, slides) aparece em stagger somente após cortina sair completamente. Fechamento é reverso exato: cortina desce cobrindo, destrói viewer atrás, restaura gallery, cortina revela gallery. Em nenhum momento gallery e viewer são visíveis simultaneamente. `.portfolio-stage` ganha `overflow:hidden` para clipar a cortina. 107 testes passando | `index.html`, `styles.css`, `script.js` |
| 01/07/2026 | Transição "viewport": substitui curtain. O viewer é a nova viewport — fundo bege (`var(--color-bg-secondary)`) cobre a seção ao deslizar de baixo (`y:100%→0%`, 850ms, `cubic-bezier(0.25,0.46,0.45,0.94)`). Galeria antiga apenas escurece (`opacity:1→0.3`) sem mover — fica para trás como segundo plano. Carrossel montado imediatamente (atrás da viewport). Conteúdo (título 20px→0, subtítulo, botão voltar, setas, slides laterais) aparece em stagger só após viewport terminar de entrar. Fechamento: conteúdo some → viewer desce → galeria clareia → cards voltam. Nenhuma cortina extra — o próprio viewer funciona como superfície de transição. 107 testes passando | `index.html`, `styles.css`, `script.js` |
| 01/07/2026 | Correção de layout do viewer: reduzidos espaçamentos internos (back button `mb:28→14px`, subtitle `mb:32→18px`, nav `mt:48→30px`). Carrossel centralizado visualmente na seção. 107 testes passando | `styles.css` |
| 01/07/2026 | **Bugfix crítico**: viewer fechava automaticamente ~1s após abrir. Causa: `stage.style.minHeight=''` no `onComplete` do `openProject` colapsava o stage (altura 0) porque gallery (`.staged`) e viewer são `position:absolute` (fora do fluxo). Com `overflow:hidden` no stage, viewer com `inset:0` ficava com altura 0 e todo conteúdo desaparecia. Correção: removido `minHeight=''` do `onComplete` — minHeight travado mantém a altura. `minHeight` só é liberado no `onComplete` do `closeProject` (quando gallery volta ao fluxo normal e stage retorna altura natural do grid). 107 testes passando | `script.js` |
| 02/07/2026 | **Bugfix definitivo**: botões de navegação (← →) do carrossel no viewer não apareciam. Causa dupla: (1) `inset:0` no CSS do viewer incluía `bottom:0`, prendendo-o à altura do stage (~500px) e cortando o nav (82px abaixo do slider de 420px); (2) `overflow:hidden` na `.portfolio-gallery-section` (seção pai) também clipava o nav. Solução: (A) CSS — viewer usa `top/left/right` sem `bottom`, com `min-height:100%` para crescer com o conteúdo; removido `overflow:hidden` da section pai. (B) JS — `requestAnimationFrame` no `onComplete` de `openProject` libera `viewer.style.bottom='auto'` e `stage.style.overflow='visible'` após GSAP finalizar estilos. No `closeProject`: restaura `overflow:hidden` no início e limpa `bottom`/`overflow` no `onComplete`. Nenhuma animação ou carrossel foi alterado. 107 testes passando | `styles.css`, `script.js` |
| 06/07/2026 | **Integração Netlify Forms**: adicionados atributos `data-netlify="true"`, `method="POST"`, `name="contato"`, campo hidden `form-name` ao formulário de contato. Substituído mock `submitToAPI` por `fetch()` real para `/` com `Content-Type: application/x-www-form-urlencoded`, `URLSearchParams` e campo `form-name` no payload. Honeypot mantido como anti-spam adicional ao Netlify. 107 testes passando | `index.html`, `script.js` |
| 06/07/2026 | **Substituição de todo o copy genérico/placeholder por conteúdo real**: seção About (2 parágrafos de apresentação + missão), 4 valores institucionais, 6 diferenciais, slogan do rodapé. Contadores zerados (`0 Projetos`/`0 Anos`/`0%`) substituídos por badges qualitativos dos 3 segmentos até haver números reais para popular (bug identificado: `parseFloat` de placeholder com colchetes em `initCounters()` sempre retorna `NaN`, fazendo a função abortar antes de animar — corrigir `data-target` para valor numérico puro quando os números existirem). Depoimentos mantidos como placeholder (aguardando depoimentos reais do cliente). **Nova seção "Segmentos"** adicionada entre Serviços e Processo: abas clicáveis Residencial/Comercial/Industrial com conteúdo próprio por aba, controladas por `initSegmentsTabs()` (função nova, chamada em `initPage()`). Carrossel do portfólio e carrossel de clientes não foram tocados — 42 testes unitários do slider passando sem alteração | `index.html`, `styles.css`, `script.js`, `README.md` |
| 13/07/2026 | **Correção de organização das fotos de obra** (auditoria AUDITORIA.md, item 1): pasta `Projeto 5/` continha 3 fotos corretas + 4 fotos nomeadas `Projeto 6 - Foto N`; pasta `Projeto 6/` continha apenas 8 fotos nomeadas `Projeto 9 - Foto 40-47` (nenhuma foto real de "Projeto 6"). Movidas as 4 fotos de `Projeto 5/` para `Projeto 6/` e as 8 fotos de `Projeto 6/` para `Projeto 9/` (sequência 40-47 encaixa sem conflito após a Foto 39 existente). Nenhuma dessas pastas é referenciada por `index.html`/`script.js` hoje — mudança sem impacto no site em produção | `Projeto 5/`, `Projeto 6/`, `Projeto 9/` |
| 13/07/2026 | **Remoção de links quebrados (404)** (auditoria AUDITORIA.md, item 2): removidas as referências a `assets/favicon.svg` e a 4 logos de clientes inexistentes (`assets/images/clients/client-05.png` a `client-08.png`), substituídas por comentários `<!-- TODO -->`. O carrossel de clientes (`initClientsCarousel` em `script.js`) lê o número de slides dinamicamente via `querySelectorAll`, então a remoção dos 4 `<li>` não quebra a lógica de duplicação/loop | `index.html` |
| 13/07/2026 | **Remoção do Swiper (dependência morta)** (auditoria AUDITORIA.md, item 3): confirmado via busca em `script.js` que nenhuma função depende do Swiper (o carrossel de portfólio já é 100% customizado via `createCascadingSlider()`/`initCascadingSlider()`). Removidos o `<link>` do CSS e o `<script>` do Swiper via CDN, eliminando 2 requisições externas desnecessárias. 107 testes passando após a remoção | `index.html` |
| 13/07/2026 | **Remoção de log com PII em produção** (auditoria AUDITORIA.md, item 4): removido `console.log('[Perin Form] Submitted to Netlify Forms:', ...)` que expunha nome, telefone e e-mail do lead no console do navegador do cliente a cada envio do formulário. Projeto não possui sistema de log próprio, então o log foi apenas removido (sem substituição) | `script.js` |
| 13/07/2026 | **Remoção de código morto** (auditoria AUDITORIA.md, item 5): confirmado via busca em todo o projeto que `diagnoseHeroState()` nunca era chamada em lugar nenhum — função de debug órfã (5 `console.log` de diagnóstico do estado da Hero). Bloco completo removido | `script.js` |
| 13/07/2026 | **SEO — meta tags de compartilhamento social** (auditoria AUDITORIA.md, item 6): adicionadas `og:image`, `og:locale` e Twitter Card completo (`twitter:card`, `twitter:title`, `twitter:description`, `twitter:image`), seguindo o padrão das tags Open Graph já existentes. Usada `portfolio-screenshot.png` (já existente no projeto) como imagem padrão até haver uma imagem dedicada para social share | `index.html` |
| 13/07/2026 | **Limpeza de CSS órfão de versões antigas da transição do portfólio**: auditadas todas as classes `.portfolio-*` em `styles.css` contra referências em `index.html`/`script.js`. Removidas `.portfolio-expanding-card`/`.portfolio-expanding-card img` (clone flutuante da abordagem "expand card" descartada, nunca criado via JS) e a regra de media query `.portfolio-swiper-nav { gap: 20px }` (resíduo do nav do Swiper original, substituído pelas setas do Cascading Slider). Classes `.staged`, `.portfolio-stage`, `.portfolio-viewer*` seguem ativas (usadas por `openProject`/`closeProject` em `script.js`) e não foram tocadas. `.surface-slide`, `.curtain`, `.gallery-dim` já não existiam mais no CSS (removidas em iterações anteriores, só citadas no histórico). 107 testes passando após a limpeza | `styles.css` |
| 13/07/2026 | **Otimização de imagens da raiz para .webp**: convertidas via `sharp-cli` (ferramenta pontual, não adicionada como dependência do projeto) as 5 fotos genéricas do portfólio placeholder (`trabalhando01-05`, qualidade 70) e os 6 logos (`eldorado`, `elektro`, `isa-energia`, `state-grid`, `LOGO PERIN PNG ATUAL01`, `portfolio-screenshot`, qualidade 82). Peso total caiu de 1649,6 KB para 1243,5 KB (**-24,6%**). Todas as referências em `index.html` (og:image, twitter:image, logos de clientes, logo do rodapé) e no array `portfolioProjects` de `script.js` (`cover`/`photos`) atualizadas para os novos arquivos `.webp`. Arquivos originais preservados em `originais-nao-webp/` (não versionado no código, apenas backup local) e removidos da raiz. `LOGO PERIN PNG ATUAL02.jpg` (usado na navbar) não foi convertido nesta rodada — avaliado separadamente. Testes unitários e de regressão que verificavam os nomes de arquivo antigos (`.png`/`.jpeg`) atualizados para os novos nomes `.webp`. 107 testes passando | `index.html`, `script.js`, `tests/unit/slider.test.js`, `tests/regression/slider.regression.test.js`, `originais-nao-webp/` (novo) |
| 13/07/2026 | **Conversão do logo da navbar para .webp**: `LOGO PERIN PNG ATUAL02.jpg` (46,3 KB) convertido via `sharp-cli` (qualidade 82) para 22,5 KB (**-51%**). Apesar de estar abaixo do limiar de 50KB geralmente considerado dispensável, a redução de metade do peso justificou a conversão por ser um elemento carregado em toda página (logo fixo do header, acima da dobra). Original movido para `originais-nao-webp/`, referência atualizada em `index.html` e no teste de regressão que verificava o nome antigo. 107 testes passando | `index.html`, `tests/regression/slider.regression.test.js`, `originais-nao-webp/` |
| 14/07/2026 | **Vídeo time-lapse de fundo no Hero** (branch `feat/hero-timelapse-video`): adicionado vídeo de obra em loop (terreno → prédio de 2 pavimentos → reverso → repete) como plano de fundo da seção Hero, plano técnico em `technical-plan.md`. Loop implementado com dois arquivos (`construction-timelapse.mp4`/`.webm` + variante `-reverse`) trocados no evento `ended`, evitando scrubbing reverso via JS (bug conhecido no Safari). Encoding via ffmpeg (H.264 baseline 3 Mbps + fallback WebM VP9), poster `.webp` de 58 KB como LCP. Fallback para poster estático quando `prefers-reduced-motion` ou conexão 2G/slow-2G. Overlay com vinheta + matiz verde-preto para contraste; título/subtítulo/CTA secundário ganharam cor clara escopada ao Hero (`#F5F5F5`) para legibilidade contra o vídeo; CTA secundário e badge com `backdrop-filter: blur(8px)`. Loop com "held moment" de 260ms no pico da construção antes de reverter; primeira reprodução pula os 2s iniciais (terreno vazio) para uma primeira impressão com atividade visível. Mobile usa `object-position: 62% center` para enquadrar a atividade em vez do céu/grama vazios. Decisões e desvios documentados em `technical-plan.md` e `implementation-notes.md`. 42 testes unitários + 65 testes de regressão passando (sem alteração ao carrossel) | `index.html`, `styles.css`, `script.js`, `technical-plan.md` (novo), `implementation-notes.md` (novo), `assets/videos/` (novo), `assets/images/hero-fundo/` |
| 13/07/2026 | **Exceção .webp em og:image/twitter:image por compatibilidade de crawlers sociais**: Facebook/WhatsApp, X/Twitter e principalmente LinkedIn têm suporte inconsistente a `.webp` em tags de preview social — risco real de o card de compartilhamento não renderizar. Gerado `social-preview.jpg` (1200×630px, dimensão padrão recomendada para Open Graph/Twitter Card, qualidade 85, 20,5 KB) a partir do `portfolio-screenshot.png` original em `originais-nao-webp/`. Tags `og:image` e `twitter:image` apontam agora para `social-preview.jpg`; adicionadas `og:image:width`/`og:image:height` (1200/630). Esta é uma exceção isolada — todo o restante do site (portfólio, logos, carrosséis) permanece em `.webp`. 107 testes passando | `index.html`, `social-preview.jpg` (novo) |
| 13/07/2026 | **Remoção de arquivo órfão**: `portfolio-screenshot.webp` ficou sem nenhuma referência no código após a criação do `social-preview.jpg` (que passou a ser usado em `og:image`/`twitter:image`). Confirmado via busca em todo o projeto que não havia referência em `index.html`, `script.js`, `styles.css` ou testes — apenas menções históricas em documentação (README, AUDITORIA.md). Arquivo removido da raiz; o PNG original permanece preservado em `originais-nao-webp/` | *(remoção)* `portfolio-screenshot.webp` |
| 13/07/2026 | **Reorganização completa da estrutura de arquivos e nomes**: (1) imagens da raiz movidas e renomeadas para `assets/images/{clients,brand,social,placeholders}/` com nomes descritivos em kebab-case (`LOGO PERIN PNG ATUAL01.webp` → `assets/images/brand/logo-perin-principal.webp`, `LOGO PERIN PNG ATUAL02.webp` → `assets/images/brand/logo-perin-navbar.webp`, `trabalhando0N.webp` → `assets/images/placeholders/placeholder-obra-0N.webp`, logos de clientes → `assets/images/clients/`, `social-preview.jpg` → `assets/images/social/`); (2) as 12 pastas de fotos reais de obras (`Projeto 1`..`Projeto 11`, `Duvidas`) movidas e renomeadas para `assets/images/portfolio/portfolio-projeto-01`..`portfolio-projeto-11` e `portfolio-duvidas`, com arquivos internos renomeados sequencialmente para `foto-01.webp`, `foto-02.webp` etc. (contagem de arquivos por pasta conferida antes/depois, nenhuma foto perdida); nomes reais das obras não foram inventados — não existiam em nenhum documento do projeto, então a numeração foi mantida com nomenclatura mais clara. Nenhuma dessas pastas de portfólio real é referenciada pelo site hoje (mesma situação de antes da reorganização) — apenas os caminhos ficaram mais organizados e descritivos. Todas as referências em `index.html`, `script.js` (`portfolioProjects`) e nos testes (`tests/unit/slider.test.js`, `tests/regression/slider.regression.test.js`) atualizadas para os novos caminhos. `SPECS.md` e `AGENTS.md` não continham caminhos de arquivo específicos — não precisaram de alteração. Busca final por todos os nomes antigos (`Projeto \d`, `Duvidas`, `LOGO PERIN PNG ATUAL0N`, `eldorado.webp` sem prefixo, `trabalhando0N`) não retornou nenhuma referência esquecida em `index.html`/`script.js`/`styles.css`/`tests/`. Nenhum arquivo foi deletado, apenas movido/renomeado. Estrutura documentada na seção "Estrutura do Projeto" acima. 107 testes passando; validação visual no navegador sem 404 | `index.html`, `script.js`, `tests/unit/slider.test.js`, `tests/regression/slider.regression.test.js`, `README.md`, `assets/` (nova estrutura) |
| 16/07/2026 | **Otimização de performance (branch `perf/otimizacao-performance`)**: Lighthouse Performance Score 81→89, LCP 3.3s→2.9s, TBT 220ms→100ms, peso da página 312→274 KiB (relatório completo em `RELATORIO-PERFORMANCE.md`). (1) Geradas variantes `-mobile.webp`/`-desktop.webp` das 5 imagens `placeholder-obra-0N.webp` usadas no carrossel de portfólio, com `srcset`/`sizes` via novo helper `buildResponsiveImgAttrs()` em `script.js` — nenhuma regra de proporção/animação/duração do carrossel foi alterada (validado com `carousel-check`, 65/65 testes de regressão passando); originais preservados em `img-originais/placeholders/`. (2) `loading="lazy"` adicionado nas imagens abaixo da dobra (logos do carrossel de clientes, logo do rodapé, fotos secundárias da galeria do portfólio). (3) `defer` adicionado aos 4 `<script>` (Bootstrap, GSAP, ScrollTrigger, `script.js`) — sem impacto, pois `script.js` já esperava `DOMContentLoaded`. (4) Google Fonts (Manrope, Inter) baixado e servido localmente em `/fonts` (apenas 2 arquivos woff2 variáveis, subset `latin`, cobre acentuação PT-BR) com `@font-face`/`font-display: swap` em `styles.css`; `<link>` externo removido; `preconnect` trocado para os CDNs remanescentes (jsdelivr, cloudflare) e `preload` adicionado para a fonte do título do hero. (5) Criado `_headers` na raiz com cache de 30 dias para CSS/JS (sem hash de build) e 1 ano/immutable para imagens e fontes. (6) Passe manual único de minificação (`terser`/`clean-css-cli`) gerando `script.min.js`/`styles.min.css` — mantidos `script.js`/`styles.css` legíveis à parte porque os testes de regressão fazem checagem de padrões no texto-fonte (proporções, curva, duração), incompatível com minificação in-place; `index.html` atualizado para referenciar os `.min`. Fontes pré-minificação preservadas em `src-original/`. Pipeline de build automatizado (esbuild/hash/netlify.toml→dist) foi avaliado e descartado por decisão consciente — fora de escopo desta rodada. 107 testes passando | `index.html`, `script.js`, `styles.css`, `script.min.js` (novo), `styles.min.css` (novo), `_headers` (novo), `fonts/` (novo), `img-originais/` (novo), `src-original/` (novo), `audit/lighthouse-antes.*`, `audit/lighthouse-depois.*`, `RELATORIO-PERFORMANCE.md` (novo) |
| 16/07/2026 | **Fechamento da branch de performance**: (1) Investigado o Bootstrap CDN — confirmado que nunca foi removido (nem parcialmente); não há vestígio local (`vendor/`, `css/`, `package.json`), então não é uma referência esquecida e sim a dependência inteira ainda via CDN. Decisão do usuário: adiar remoção/self-host para uma branch separada, não tocado aqui. (2) Tentativa de mover `script.js`/`styles.css` (fonte legível) para `/dev-source/` por organização — revertida imediatamente ao confirmar que `tests/regression/slider.regression.test.js:18` lê esses arquivos por caminho fixo na raiz; sem alterar os testes, a suíte inteira quebra (0/65). Nenhum `netlify.toml` foi criado, arquivos voltaram ao lugar original. 107 testes passando | `RELATORIO-PERFORMANCE.md` |
| 16/07/2026 | **Remoção completa do Bootstrap (CSS+JS via CDN)**: investigação sistemática (`audit/bootstrap-uso-real.md`) confirmou 0% de uso real — grep por todas as classes de `index.html` contra os padrões do Bootstrap (`.container`, `.row`, `.col-*`, `.btn`, `.navbar`, `.card`, utilities) não encontrou nenhuma correspondência real (as coincidências de nome, como `.nav-link`, são classes próprias do projeto com estilo independente em `styles.css:220`); busca por `data-bs-*` e chamadas `bootstrap.Modal/Tooltip/Dropdown/etc` em `script.js`/`index.html` também retornou zero. Removidos de `index.html`: `<link rel="preconnect" href="https://cdn.jsdelivr.net">`, `<link rel="stylesheet">` do Bootstrap CSS e `<script defer>` do bundle JS (que incluía Popper — sem uso isolado de Popper encontrado). Validação visual sistemática via Puppeteer (antes/depois, desktop 1440px e mobile 375px, 8 pontos de scroll, menu mobile aberto, formulário completo) não encontrou nenhuma diferença — projeto já tinha reset próprio (`box-sizing: border-box`, `styles.css:76-78`), então o Reboot do Bootstrap não tinha efeito visual sendo consumido. Performance Score 89→98, LCP 2.9s→2.4s, TBT 100ms→50ms, peso 274→217 KiB, 2 requisições a menos. 107 testes + 65 testes de regressão passando, nenhuma regressão de score em nenhum audit do Lighthouse (comparação programática). Detalhes em `RELATORIO-PERFORMANCE.md` (seção "Rodada 2") e `audit/proximas-oportunidades.md` | `index.html`, `RELATORIO-PERFORMANCE.md`, `audit/bootstrap-uso-real.md` (novo), `audit/proximas-oportunidades.md`, `audit/lighthouse-pos-bootstrap.*` (novo) |
| 29/07/2026 | **Redesign dark-theme da seção CTA principal** (`#cta-principal`), a partir do projeto Claude Design importado `CTA Final - Perin.dc.html`: fundo trocado de verde sólido para preto (`var(--cor-preto-puro)`) com glow radial verde (`radial-gradient` sutil atrás do conteúdo), adicionado texto-fantasma decorativo "VERDADE" em outline (`-webkit-text-stroke`, `color:transparent`, `pointer-events:none`) posicionado atrás do heading via `position:relative`/`z-index`, badge convertido para pill com fundo/borda com tingimento do verde de destaque, cor do texto em destaque (`<em>`) trocada para `var(--color-accent-glow)`. Tokens de design (`--cor-preto-puro`, `--color-accent-glow`, fontes) já existiam em `:root` — não foi necessário buscar o design system remoto (`_ds_bundle.css`/`.js`) para resolver valores. Botões, subtítulo e selos de confiança mantidos sem alteração (já compatíveis com o novo fundo escuro). Verificação visual em navegador (desktop 1440px) confirmou fundo preto, glow, ghost text e badge renderizando corretamente; breakpoints mobile/tablet herdam a mesma regra base sem alteração adicional. **Bug incidental encontrado e corrigido**: `index.html` carrega `styles.min.css` (não `styles.css`) em produção/local — o arquivo minificado não havia sido regenerado após a edição, então o CTA continuava renderizando com o CSS antigo até rodar `npx clean-css-cli styles.css -o styles.min.css` e validar com `scripts/check-min-freshness.js` | `index.html`, `styles.css`, `styles.min.css` |
| 16/07/2026 | **Otimização dos 3 logos superdimensionados** (últimos itens pendentes de `audit/proximas-oportunidades.md`): confirmado tamanho de exibição real no CSS (`logo-perin-navbar.webp`: 40px altura fixa, `styles.css:203-208`; `elektro.webp`/`state-grid.webp`: card 215×152px do carrossel de clientes, `styles.css:1770-1791`, mesmo em todos os breakpoints). Redimensionados via `sharp-cli` para 2x o bounding box real (suporte a retina sem exagero), qualidade 92 (mais conservadora que as fotos de portfólio, por serem logos com texto fino): `logo-perin-navbar.webp` 935×229→330×81px (-58,5%), `elektro.webp` 1500×898→400×239px (-65,4%), `state-grid.webp` 721×218→400×121px (-7,9%, arquivo já eficiente, ganho veio da resolução). Total: 63,3 KB → 36,5 KB (-42,3%). Canal alpha testado e preservado onde existia (`state-grid`, `logo-perin-navbar`). Substituição in-place (mesmo nome de arquivo) — nenhuma referência em `index.html`/`script.js` precisou mudar. Originais preservados em `img-originais/brand/` e `img-originais/clients/`. Validação visual (Puppeteer, desktop+mobile) confirmou nitidez sem perda perceptível. 107 testes passando. Preparadas em `RELATORIO-PERFORMANCE.md` as instruções de validação de cache pós-deploy (pendente, requer deploy real — `npx serve` local não aplica `_headers`) | `assets/images/brand/logo-perin-navbar.webp`, `assets/images/clients/elektro.webp`, `assets/images/clients/state-grid.webp`, `RELATORIO-PERFORMANCE.md`, `img-originais/brand/` (novo), `img-originais/clients/` (novo), `audit/lighthouse-pos-logos.*` (novo) |
| 26/07/2026 | **Endereço + mapa no rodapé**: adicionado `<address>` abaixo da tagline do footer com o endereço da empresa (Rua Dr. Orensy Rodrigues da Silva, 917 — Sala 02, Centro, Andradina/SP, CEP 16901-003) e um `<iframe>` do Google Maps embutido sem chave de API (`maps.google.com/maps?q=...&output=embed`), `loading="lazy"`. Novas classes `.footer-endereco`/`.footer-mapa-container`/`.footer-mapa` em `styles.css`, com `max-width:100%` do mapa em mobile. Validado visualmente em desktop (1440px); resize real da janela do Chrome automatizado não funcionou neste ambiente, então mobile foi validado por análise estática do CSS (sem larguras fixas em px) | `index.html`, `styles.css`, `styles.min.css` |
| 26/07/2026 | **Simplificação e reversão parcial do Portfólio**: primeira tentativa substituiu a galeria+viewer por um carrossel flex simples sem GSAP — revertida a pedido do usuário por descaracterizar o visual (card central desproporcional). Versão final: o Cascading Slider (Modus) já existente — `initCascadingSlider()`, CSS `.cascading-slider*`, card central ~60% da largura com laterais visíveis via `clip-path`/GSAP — foi mantido intacto e passou a **ser** a própria galeria do portfólio, populado diretamente com os 6 projetos (1 foto de conclusão + nome + tipo/ano por slide, novo `.cascading-slider__item-overlay`), em vez de fotos de um projeto aberto num viewer. Removidos por completo: `openProject()`/`closeProject()` (transição de viewport para o viewer), `initPortfolioGallery()` (grid de seleção `#portfolioGrid`), `portfolioState`, e todo o HTML/CSS de `.portfolio-stage`/`.portfolio-gallery*`/`.portfolio-viewer*`/`.portfolio-card*`. Clicar num card lateral usa o `goTo()` já existente no engine — só navega aquele projeto ao centro, nunca abre uma segunda tela. Nova função `initPortfolioSlider()` monta o slider uma única vez no load da página (fila idle de `initPage()`). Suítes de teste do portfólio reescritas para a nova estrutura — 80 testes passando | `index.html`, `styles.css`, `script.js`, `tests/unit/slider.test.js`, `tests/regression/slider.regression.test.js`, `tests/unit/init-scheduling.test.js` |
| 29/07/2026 | **Recomposição total da seção CTA principal** (`#cta-principal`), a partir da revisão final da pasta local `Redesign CTA Perin Construções/` (export atualizado do canvas Claude Design, mais recente que o fetch anterior via DesignSync que gerou o registro de 29/07/2026 acima): layout deixou de ser centralizado — conteúdo alinhado à esquerda (`max-width:600px`, `padding` assimétrico), glow verde radial (`.cta-principal-glow`, novo elemento) movido do centro para o canto superior-direito, texto-fantasma "VERDADE" reposicionado para o canto superior-esquerdo (antes centralizado). Badge "Próximo passo" convertido de pill com fundo para texto plano verde com letter-spacing. Removido o destaque itálico serifado (`<em class="cta-principal-destaque">`) — heading agora é uma única frase em branco/bold sem palavra de ênfase. Os dois botões (`.cta-principal-btn-primario`/`.cta-principal-btn-secundario`, com estilos visualmente distintos) foram substituídos por uma única classe `.cta-principal-btn` (efeito "glass": fundo translúcido + `backdrop-filter: blur`), aplicada igualmente aos dois — visual idêntico entre "Solicitar Orçamento" e "Falar pelo WhatsApp", ícone de chat trocado de emoji (`💬`) para SVG inline (paridade com o export do design). Removida a linha de selos de confiança (`.cta-principal-selos`/`.cta-principal-selo`, não presente no design final). Breakpoints desktop (>1024px)/tablet (≤1024px)/mobile (≤480px) recalibrados com os valores exatos do export (tamanhos de fonte, paddings, blur do glow) para as 3 larguras de referência do canvas (1180px/744px/390px). `styles.min.css` regenerado via `npx clean-css-cli` e validado com `scripts/check-min-freshness.js`. 80 testes passando | `index.html`, `styles.css`, `styles.min.css` |

---

## 📌 Observabilidade (Logs)

- Não há `console.log` no código de produção
- O formulário de contato envia dados para o Netlify Forms via `POST` com `Content-Type: application/x-www-form-urlencoded`
- Nenhuma PII (informação pessoal) é coletada ou processada além do que o usuário preenche no formulário

---

© 2026 Perin Construções. Todos os direitos reservados.